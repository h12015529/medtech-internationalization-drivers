...
if "curcd" in df.columns:
    n_before = len(df)
    # EUR filter removed — all currencies kept.
    # ROA, R&D intensity and leverage are currency-neutral ratios.
    print(f"\nCurrencies in sample: {df['curcd'].unique().tolist()}")
...
if "ib" in df.columns and "at" in df.columns:
    df["roa"] = df["ib"] / df["at"]
if "xrd" in df.columns and "at" in df.columns:
    df["xrd_intensity"] = df["xrd"].fillna(0) / df["at"]
if all(c in df.columns for c in ["dltt", "dlc", "seq"]):
    df["leverage"] = (df["dltt"] + df["dlc"]) / df["seq"]
print("Research variables constructed: roa, xrd_intensity, leverage")
# -- Save ----------------------------------------------------------------------
print(f"\nSaving to {OUT_PATH} ...")
