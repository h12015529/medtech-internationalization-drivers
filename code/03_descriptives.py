...
Sample
------
Listed medical-technology firms (Compustat Global, SIC 3841 & 3845), 2015–2025.

Note on DOI
-----------
pifo (foreign income) is not available in this Compustat Global pull.
rect/sale produces extreme outliers for firms with low/volatile sales.
...
# ── Micro-firm filter ──────────────────────────────────────────────────────────
n = len(df)
df = df[df["at"] >= 1].copy()   # remove assets < EUR 1m (negative log(at) distorts size plots)
print(f"  After micro-firm filter (at>=1): {len(df):,} (removed {n-len(df):,})")
# No SME size filter — sample is listed medical-technology firms (SIC 3841 & 3845)
...
print("\nWinsorizing at 1%-99%...")
for col in ["roa", "rd_intensity", "leverage", "capx_intensity",
            "cash_ratio"]:
    df[col] = winsorize(df[col])
    print(f"  {col:<20} [{df[col].min():>8.4f}, {df[col].max():>8.4f}]")

# EBITDA margin: extreme outliers from tiny-sales firms -> winsorize tighter (5%-95%)
df["ebitda_margin"] = winsorize(df["ebitda_margin"], lower=0.05, upper=0.95)
print(f"  ebitda_margin (5-95%) [{df['ebitda_margin'].min():>8.4f}, {df['ebitda_margin'].max():>8.4f}]")
...
# Left: scatter + bin means — leverage vs RoA (all firms)
axes[0].scatter(df_plot["leverage"], df_plot["roa"],
                alpha=0.1, s=8, color=WU_BLUE, label="Firm-years")
...
# Right: median RoA by firm size — Low vs High leverage (H2 preview)
lev_median = df_plot["leverage"].median()
df_plot["lev_group"] = np.where(df_plot["leverage"] <= lev_median,
